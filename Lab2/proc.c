...
*/
	...
	for(;;){
		...
		if(total_nice > 0) {
			rand_number = rand_number % total_nice + 1;
			total_nice = 0;

			for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
				if(p->state != RUNNABLE) {
					total_running += p->nice_value;
					continue;
				}

				if((count + p->nice_value) < rand_number) {
					count += p->nice_value;
					continue;
				}
...
int
renice(int nice_value, int pid)
{
  int i = 0;
  int flag = 0;
  if(nice_value < 1 || nice_value > 50) {
  ...
  for(i = 0; i < NPROC; ++i) {
    if(ptable.proc[i].state != UNUSED) {
      if(ptable.proc[i].pid == pid) {
        ptable.proc[i].nice_value = nice_value;
				flag = 1;
      }
    }
  }
  release(&ptable.lock);
  if(flag == 0) {
    cprintf("Given pid does not exist\n");
    return 2;
  }
  return 0;
}
...
proc_cps(void)
{
	...
	acquire(&ptable.lock);
	cprintf("pid\tppid\tname\tstate\tsize");
#ifdef PROC_TIMES
...
// error this is an excellent place to add some new data to the o/p of cps
			buffer = ptable.proc[i].begin_date.year;
			if(buffer - 9 <= 0) {
				cprintf("\t0%d-", ptable.proc[i].begin_date.year);
			}
			else {
				cprintf("\t%d-", ptable.proc[i].begin_date.year);
			}
			buffer = ptable.proc[i].begin_date.month;
			if(buffer - 9 <= 0) {
				cprintf("0%d-", ptable.proc[i].begin_date.month);
			}
			else {
				cprintf("%d-", ptable.proc[i].begin_date.month);
			}
			buffer = ptable.proc[i].begin_date.day;
			if(buffer - 9 <= 0) {
				cprintf("0%d ", ptable.proc[i].begin_date.day);
			}
			else {
				cprintf("%d ", ptable.proc[i].begin_date.day);
			}
			buffer = ptable.proc[i].begin_date.hour;
			if(buffer - 9 <= 0) {
				cprintf("0%d:", ptable.proc[i].begin_date.hour);
			}
			else {
				cprintf("%d:", ptable.proc[i].begin_date.hour);
			}
			buffer = ptable.proc[i].begin_date.minute;
			if(buffer - 9 <= 0) {
				cprintf("0%d:", ptable.proc[i].begin_date.minute);
			}
			else {
				cprintf("%d:", ptable.proc[i].begin_date.minute);
			}
			buffer = ptable.proc[i].begin_date.second;
			if(buffer - 9 <= 0) {
				cprintf("0%d", ptable.proc[i].begin_date.second);
			}
			else {
				cprintf("%d", ptable.proc[i].begin_date.second);
			}
			cprintf("\t%d\t%d\t%d"
